<section class="wrapper bg-navy pt-16" style="background-image: linear-gradient(rgba(0,0,0,0.7), rgba(0,0,0,0.7)),url({{asset('assets2/img/photos/bg17.png')}});background-repeat: no-repeat;background-size: cover;">
    <div class="container pt-10 pb-17 pt-md-14 text-center">
        <div class="row">
            <div class="col-lg-8 mx-auto mb-5">
                <h3 class="display-1 mb-6 text-white">{{__('theme2-global.what-i-do-title')}}</h3>
                <p class="lead fs-18 mb-5 text-white-50 text-center">{{__('theme2-global.what-i-do-text')}}</p>
                <a href="{{route('register')}}" class="btn btn-lg btn-violet rounded-pill">{{__('theme2-global.header-button')}}</a>
            </div>
            <!-- /column -->
        </div>
        <!-- /.row -->
    </div>
    <!-- /.container -->
</section>
